...
const AllUsers = () => {
    const { data: users = [],  refetch} = useQuery(['users'], async ()=>{
        const res = await fetch('https://sakib-cricket-academy-server-noman1802.vercel.app/users')
        return res.json()
    })
    console.log(users.role);

    const handleMakeAdmin = user =>{
        fetch(`https://sakib-cricket-academy-server-noman1802.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
        .then(res => res.json())
        .then(data => {
    ...
    const handleMakeInstructor = user =>{
        fetch(`https://sakib-cricket-academy-server-noman1802.vercel.app/users/instructor/${user._id}`, {
            method: 'PATCH'
        })
        .then(res => res.json())
        .then(data => {
